					...
					<div v-else>
						...
						<div class="cart-table">
							<div class="cart-group">
								<!--购物列表-->
								<div class="cart-top-items" v-for="(item,index) in goodsShow">
									<div class="cart-items">
										<div class="name hide-row">
											<div class="name-table">
												<a href="javascript:;" target="_blank">{{item.GoodsName}}</a>
											</div>
										</div>
										<div class="operation">
											<a class="items-delete-btn" @click="delCarPanelHandle(item.Id)"></a>
										</div>
										<div class="subtotal">¥ {{item.TotalPrice}}.00</div>
										<div class="item-cols-num">
											<div class="select js-select-quantity">
												<span class="down" :class="{' down-disabled':item.Quantity<=1}" @click="subCarPanelHandle(item.Id)">-</span>
												<span class="num">{{item.Quantity}}</span>
												<span class="up" :class="{' up-disabled':item.Quantity>=item.limit_num}" @click="plusCarPanelHandle(item.Id)">+</span>	
											</div>
										</div>
										<div class="price">¥ {{item.UnitPrice}}.00</div>
									</div>
								</div>
							</div>
						</div>
...
<script>
  export default {
    ...
    methods: {
      //删除
      async delCarPanelHandle (id) {        
        try {      
          const dataParams = { Data: {Id:id,} };
          const response = await this.$axios.post('/api/UserShoppingCart/Del', dataParams);
      ...
      getDataList() {        
        ...
        const params ={
            ...
            PageSize:20,
            StartIndex:0,
            Criteria:`UserCode="${usercode}" And IsValid=True`,
        }};

        this.$axios.get("/api/UserShoppingCart/QueryEntityList", {params}).then(response => {
            //console.log('UserShoppingCart', response); 
            if (response && response.data && response.data.Data && response.data.Data.Page) {
                ...
                //this.SumTotle=lists.SumTotle(p=>p.TotalPrice);
                this.goodsShow = lists;       
                this.count= response.data.Data.TotalCount; 
                this.SumTotle=0;
                lists.forEach(element => {
                    this.SumTotle+=element.TotalPrice;
                });
            } else {
                this.goodsShow = [];
                this.count= 0;
